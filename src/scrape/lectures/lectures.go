...
type Lecture struct {
	...
	Price          string              // 수강료
	Count          string              // 강좌횟수
	Status         ReceptionStatusType // 접수상태
	DetailPageUrl  string              // 상세페이지
	ScrapeExcluded bool                // 필터링에 걸려서 파일 저장시 제외되는지의 여부(csv 파일에 포함되지 않는다)
}

// 접수상태
type ReceptionStatusType uint

// 지원가능한 접수상태 값
const (
	ReceptionStatusUnknown                   ReceptionStatusType = iota // 알수없음
...
// 지원가능한 접수상태 문자열
var ReceptionStatusString = [ReceptionStatusMax]string{"알수없음", "접수가능", "접수마감", "대기신청", "방문상담", "방문선착순", "당일참여"}
